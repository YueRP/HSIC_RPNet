...
a=load(data_name);
% Data = a.paviaU;%paviaU
Data = a.KSC;%indain pines
[row,col,num_feature] = size(Data);
a=load(data_gt);
% Label = reshape(double(a.paviaU_gt),row*col,1);%paviaU
Label = reshape(double(a.KSC_gt),row*col,1);%indian pines
num_class = max(Label(:));
clear a;

% train_num_array = [548,540,392,542,256,532,375,514,231];%paviaU
% train_num_array = [30, 150, 150, 100, 150, 150, 20, 150, 15, 150, 150, 150, 150, 150, 50, 50];%india pines
train_num_array=[33,23,24,24,15,22,9,38,51,39,41,49,91];%KSC
