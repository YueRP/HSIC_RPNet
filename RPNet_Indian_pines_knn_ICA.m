...
for l=1:Layernum
    ...
    if l==1
        ...
        XPCA = fastICA(reshape(Data, row * col, num_feature),num_PC);
        ...
        XPCAvector = fastICA([StackFeature{l}.feature],num_PC);
        minZ = min(XPCAvector);
        maxZ = max(XPCAvector);
        XPCAvector = bsxfun(@minus, XPCAvector, minZ);
        XPCAvector = bsxfun(@rdivide, XPCAvector, maxZ-minZ);
        ...
        XPCA = fastICA(StackFeature{l-1}.feature,num_PC);
...
toc
disp(['OA= ',num2str(OA),' kappa=',num2str(Kappa),' time=',num2str(toc)]);
